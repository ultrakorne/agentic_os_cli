...
- **Cron PATH is inherited from the user's interactive shell.** Cron's own PATH is minimal, so the wrapper shells out to `"$SHELL" -ilc 'printf %s "$PATH"'` and prepends that onto `$PATH`. `-i` is required so the user's rc file runs and adds dirs like `~/.local/bin` (where `claude` lives). If the shell launch fails, the wrapper falls back to a static `$HOME/.local/bin:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin`.
