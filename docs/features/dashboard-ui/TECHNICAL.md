...
- **Top-bar `rescan` is the manual discovery trigger.** The agents directory is otherwise scanned only at `engine.start()` and on the existing `EmptyAgents` / `SystemBanner` rescan affordances — nothing watches it live. The top-bar button calls `agents:rescan`, which re-runs `engine.refreshScripts()` (re-reading every `<id>.meta.json` sidecar) and broadcasts `scheduler:changed`. Useful after dropping a new script, hand-editing a sidecar, or moving an agent between section folders.
...
- **Missed runs appear inline in the detail run history.** Missed scheduled slots come back from `aos runs --json` as `JobRun{status:"missed"}` entries, so `RunHistoryList` renders them through the same `RunRow` as any other run — no separate selector or component. `RunRow`'s `status === 'missed'` branch styles the row with a dashed border, `▽` glyph, and `missed` badge; the row is read-only (no expand toggle, no `.out` to fetch). Older miss records stay in history forever; the dashboard's top banner clears them from "needs attention" the moment a newer real run lands.
