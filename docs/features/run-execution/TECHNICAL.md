...
| `internal/resources/wrapper.sh` | The shell wrapper executed by the platform backend and by `aos run` (start/end/exit/output capture); traps SIGTERM/SIGINT to write an `interrupted by reload` error record |
...
Optional fields use pointers so JSON round-trips as `null` instead of dropping or zero-defaulting. The cron-era `scheduleId` field is gone — there is no separate concept of "scheduled slot id" attached to a run. The only triggers now are `schedule` (the backend fired the wrapper) and `manual` (`aos run`).
...
The 4th arg is optional. Backend-fired runs omit it so the wrapper mints `<unix-ms>-<rand4>` itself; `aos run` passes an explicit id so the in-process stub matches the on-disk file. Trigger is conveyed via the `AGENTIC_OS_TRIGGER` env var (defaults to `"schedule"` inside the wrapper, set to `"manual"` by `aos run`). The wrapper also exports `AGENTIC_OS_DATA_DIR`, `AGENTIC_OS_AGENT_ID`, `AGENTIC_OS_AGENT_SCRIPT`, `AGENTIC_OS_RUN_ID`, and `AGENTIC_OS_TRIGGER` for the agent script.

## Noteworthy Behavior

- **Run ids are minted ahead of the spawn.** `NewRunID` produces `<unix-ms>-<rand4>`. Threading it as the wrapper's 4th argv (and into `AGENTIC_OS_RUN_ID`) is how the stub `aos run` prints matches the on-disk file. Without an explicit id the wrapper would mint its own (which is what backend-fired runs rely on, since the backend has no way to mint one ahead of time).
...
- **`EstimateRunDuration` averages the newest 10 successful runs.** Only `StatusSuccess` records with a parseable `endedAt` count — `error`, `running`, and `missed` runs are skipped so a fast-failing script can't drag the ETA below the typical successful runtime. If none qualify, the function returns `(0, false, nil)` and callers print `"none"` / emit `-1`. The estimate is rounded to 100 ms in human output for clean display.
...
- **SIGTERM/SIGINT trap writes a terminal record.** launchd's `bootout` and systemd's `disable --now` SIGTERM in-flight wrappers when refresh updates an entry; the wrapper's trap fires `write_meta error "$END" 143 "interrupted by reload"; exit 143` so the run record reflects the interruption instead of leaving an orphaned `running` row. If the trap doesn't fire (process killed -9, OOM, power loss), the next `aos tick`'s stale-running sweep rewrites the orphan an hour later.
