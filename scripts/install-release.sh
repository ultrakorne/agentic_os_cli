#!/usr/bin/env bash
# Install the latest released aos binary from GitHub.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/ultrakorne/agentic_os_cli/master/scripts/install-release.sh | bash
#
# Env overrides:
#   AOS_VERSION=v0.2.0      # pin a specific tag (default: latest)
#   BIN_DIR=/usr/local/bin  # install location (default: $HOME/.local/bin)
#
# Once installed, future updates will use the built-in `aos update` (TBD).

set -euo pipefail

REPO="ultrakorne/agentic_os_cli"
BIN_NAME="aos"
...
# Resolve tag: AOS_VERSION wins; otherwise hit /releases/latest.
tag="${AOS_VERSION:-}"
if [ -z "$tag" ]; then
  tag=$(curl -fsSL \
...
asset="aos_${version}_${os}_${arch}.tar.gz"
base="https://github.com/${REPO}/releases/download/${tag}"

tmp=$(mktemp -d 2>/dev/null || mktemp -d -t aos-install)
trap 'rm -rf "$tmp"' EXIT
...
if [ -f "$HOME/.config/aos/config.toml" ]; then
  printf 'next: run `aos refresh` to rebuild the crontab with the new binary path.\n'
fi
