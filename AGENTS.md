...
- `src/main/` — Electron main:
  ...
  - `scheduler/runs-store.ts` — watches `<aos_home>/runs/` and asks `aos runs --json --limit N` for the snapshot; `.out` reads stay as plain `fs.readFile` (view-only)
  - `scheduler/misses-store.ts` — watches `<aos_home>/misses/` (no CLI verb yet; the directory itself is the contract)
  - `theme/`, `ipc.ts`
